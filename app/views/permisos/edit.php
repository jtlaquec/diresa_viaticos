<div style="display: flex; justify-content: center;" class="text-dark">
    ...
    <form action="<?php echo BASE_URL; ?>/permisos/<?php echo $data['IdUserNivel']; ?>" method="POST" style="width: 40%; background-color: #f8f9fa; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            ...
            <select class="js-example-basic-single form-control  py-2" name="usuario_IdUser" required>
                ...
                if (!empty($empleados)) {
                    foreach ($empleados as $empleado) {
                        $selected = ($empleado['IdUser'] == $data['usuario_IdUser']) ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($empleado['IdUser']) . '" ' . $selected . '>' . htmlspecialchars($empleado['usuario']) . '</option>';
                    }
                } else {
            ...
            <select class="js-example-basic-single form-control  py-2" name="nivel_IdNivel" required>
                ...
                if (!empty($niveles)) {
                    foreach ($niveles as $nivel) {
                        $selected = ($nivel['IdNivel'] == $data['nivel_IdNivel']) ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($nivel['IdNivel']) . '" ' . $selected . '>' . htmlspecialchars($nivel['nivel']) . '</option>';
                    }
                } else {
