<div class="text-center">
    <input type="text" id="searchInput" placeholder="Buscar Empleados..." style="width: 600px; border: 2px solid gray; color:black;" autocomplete="off">
    <div class="center py-3">
        <a href="<?php echo BASE_URL; ?>/empleados/create">
            <button class="btn btn-secondary">Crear Nuevo Empleado</button>
        </a>
    </div>
</div>

<table id="empleadosTable" class="">
    <thead>
        <tr>
            <th class="primera_fila center">Id</th>
            <th class="primera_fila center">DNI</th>
            <th class="primera_fila center">C. Laboral</th>
            <th class="primera_fila center">Nombre</th>
            <th class="primera_fila center">Banco</th>
            <th class="primera_fila center">Cta. Cte</th>
            <th class="primera_fila center" colspan="2" style="width: 10px;">Acciones</th>
        </tr>
    </thead>
    <tbody id="empleadosBody">
        <?php foreach ($data as $row) : ?>
            <tr>
                <td class="segunda_fila center" style="width: 44px; height:50px;"><?php echo $row["IdPer"]; ?></td>
                <td class="segunda_fila center" style="width: 90px;"><?php echo $row["dni"]; ?></td>
                <td><?php echo $row["condicion_laboral"]; ?></td>
                <td><?php echo $row["nombre"]; ?></td>
                <td><?php echo $row["banco"]; ?></td>
                <td><?php echo $row["ctacte"]; ?></td>
                <td>
                    <button type="button" onclick="confirmDelete('<?php echo BASE_URL; ?>/empleados/<?php echo $row['IdPer']; ?>')" class="btn btn-danger">Borrar</button>
                </td>
                <td>
                    <a href="<?php echo BASE_URL; ?>/empleados/<?php echo $row['IdPer']; ?>/edit">
                        <button type="button" class="btn btn-primary">Actualizar</button>
                    </a>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>

<div class="pagination py-4" id="pagination"></div>

<form id="delete-form" action="" method="POST" style="display: none;">
    <input type="hidden" name="_method" value="DELETE">
</form>