<div style="display: flex; justify-content: center;" class="text-dark">
    <form action="<?php echo BASE_URL; ?>/condicion_laboral" method="POST" style="width: 40%; background-color: #f8f9fa; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
        <div class="form-group py-2">
            <label for="condicion_laboral" class="fw-bold py-2">Condición Laboral</label>
            <input type="text" id="condicion_laboral" name="condicion_laboral" class="form-control" required placeholder="Ingrese la Condición Laboral">
        </div>
            ...
            <select class="js-example-basic-single form-control  py-2" name="ley_laboral_IdLlab" required>
                ...
                if (!empty($leyes_laborales)) {
                    foreach ($leyes_laborales as $ley_laboral) {
                        echo '<option value="' . htmlspecialchars($ley_laboral['IdLlab']) . '">' . htmlspecialchars($ley_laboral['ley_laboral']) . '</option>';
                    }
                } else {
